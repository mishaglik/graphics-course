...
namespace ImGui
{
    ...
    float BezierValue(float dt01, float P[4]) {
        ...
        return results[(int) ((dt01 < 0 ? 0 : dt01 > 1 ? 1 : dt01) * int(STEPS))].y;
    }
    ...
    int Bezier(const char *label, float P[5]) {
        ...
        const float dim = AREA_WIDTH > 0 ? float(AREA_WIDTH) : avail;
        ...
        //     int idx = (int) (delta * (int)SMOOTHNESS);
        ...
        DrawList->AddCircleFilled(p1, (int)GRAB_RADIUS, ImColor(white));
        DrawList->AddCircleFilled(p1, (int)GRAB_RADIUS - (int)GRAB_BORDER, ImColor(pink));
        DrawList->AddCircleFilled(p2, (int)GRAB_RADIUS, ImColor(white));
        DrawList->AddCircleFilled(p2, (int)GRAB_RADIUS - (int)GRAB_BORDER, ImColor(cyan));
