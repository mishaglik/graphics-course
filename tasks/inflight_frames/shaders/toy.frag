...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 scale = 9.0 * UniParams.resolution.xy / max ( UniParams.resolution.x, UniParams.resolution.y ) ;
    vec2 uv    = scale * ( fragCoord / UniParams.resolution.xy - vec2 ( 0.5 ) );
    ...
    float iTime = pushConstant.iTime;
        ...
        {       
            ...
            color = surf * getLight(pr, light, m, generateNormal( p, 1e-6, m ));
            ...
            if (dot(color, vec4(1,1,1,0)) < 0.1) {
                color =  0.05 * ambientOcclusion(p, generateNormal( p, 1e-5, m ), m) * surf;
            }
        } else {
            color = texture(cubeMap, dir);
        }
        float flash = pow(max(0., dot(normalize(UniParams.cam_view), dir)), 3.);
        color += flash * vec4(1, 1, 1, 1);
...
void main()
{
  ...
  ivec2 uv = ivec2(vec2(surf.texCoord.x, 1. - surf.texCoord.y) * vec2(UniParams.resolution));
  mainImage(color, uv);
}
